...
inspect, search, depends graph, and maintenance, with mark, claim, create, meta, and order writes
...
- `internal/` — pure wire-contract primitives, then the engines built on them:
  ...
  - `tkv` — localhost HTTP dashboard (templates, static CSS, goldmark inspect, depends graph, mark/claim/create/meta/order, chrome lens, sync)
