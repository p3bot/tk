...
P1 through P7 have landed, plus `tkv` (reads plus mark/claim/lens). `tk` runs as a Cobra CLI with the machine-local CUE
...
- `internal/` — pure wire-contract primitives, then the engines built on them:
  ...
  - `tkv` — localhost HTTP dashboard (templates, static CSS, goldmark inspect, depends graph, mark/claim, chrome lens)
